...
const Confirm = ({ values }) => {
  ...
  const POST = async () => {
    try {
      ...
      const respDocument = await fetch(`http://localhost:8080/document/create`, {
        method: 'POST',
      ...
      if (respDocument.ok) {
        ...
        const respCustomer = await fetch(`http://localhost:8080/customer/create/${document.number}`, {
          method: 'POST',
          headers: { 'Content-Type': 'Application/json' },
        ...
        if (respCustomer.ok) {
          ...
          const respAccount = await fetch(`http://localhost:8080/account/create/${document.number}`, {
            method: 'POST',
            headers: { 'Content-Type': 'Application/json' },
          ...
          if (respAccount.ok) {
            ...
            // Request POST
            const request = {
              fundSource: values.sourceFund,
              fundArrival: values.arrivalFund,
            ...
            const respRequest = await fetch(`http://localhost:8080/request/create/${values.accountNumber}/${values.officeId}`, {
              method: 'POST',
              headers: { 'Content-Type': 'Application/json' },
            ...
            if (respRequest.ok) {
              ...
              const complement = {
                ...
                accountUse: values.dedicatedUse,
              }
              ...
              const foreignUse = {
                ...
                source: values.sourceForeignFund,
                arrival: values.arrivalForeignFund,
              }
  ...
  const generatePDF = () => {
    ...
    doc.setFontSize(11);
    doc.setFont("helvetica", "normal");
    doc.text(`${values.agencyAddress}`, 20, 80);
            ...
            <Grid item xs={12}>
              <ListItem>
                <ListItemText
                  ...
                  secondary={values.agencyAddress}
                />
              </ListItem>
            </Grid>
            <Grid item xs={12}>
